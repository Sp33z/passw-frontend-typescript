    ...
    <>
      ...
      >
        {ONBOARDING_SLIDES.map((options) => {
          ...
          return <SwiperSlide key={id} className="flex! flex-col! items-center! justify-evenly! gap-8">
            ...
            <Image className="w-auto h-1/2 my-auto" loading="eager" src={visual} alt="Phone with lock." />
          </SwiperSlide>
        })}
      </Swiper>
      ...
      <footer className="w-full h-fit flex flex-row  items-center justify-center gap-4">
          <button className="bg-transparent w-2/5 h-fit flex items-center justify-center py-4 px-8 rounded-full" onClick={() => { swiper?.slideTo(swiper.slides.length) }}>
              <p className="font-semibold text-(--text)" style={{opacity: ((swiper?.slides.length || 0) - 1) == activePage ? "0.25" : "1"}}>Skip</p>
          </button>
