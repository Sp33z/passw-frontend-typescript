...
const ProgressBar = (props: ElementProps) => {
  ...
  return (
    <div className="w-full h-fit flex flex-col bg-(--background) text-(--text) shadow gap-3 p-3 rounded-xl">
      ...
      <div className="relative w-full h-4 bg-(--text)/10 rounded-full overflow-hidden">
        <div
          className="h-full bg-(--text) duration-300 rounded-full"
        ...
        {options.map((option, index) => {
          return (
            <Status
              ...
              active={(100 / options.length) * (index + 1)}
            />
          );
        })}
      </div>
